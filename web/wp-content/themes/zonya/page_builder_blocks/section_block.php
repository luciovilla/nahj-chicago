...
?>
		...
		<div class="one_third last">	
			...
			<p class="description">
				<label for="<?php echo $this->get_field_id('thin') ?>">
					Use thin content width for this section?
					<?php echo aq_field_checkbox('thin', $this->block_id, $thin) ?>
				</label>
			</p>
		</div>
		<div class="clear"></div>
...
<?php
	...
	?>
		...
		<div class="ebor-column-header">
			<a href="#" class="column-close">Show/Hide Details<code><?php echo $title; ?></code></a>
	...
	function block_callback($instance) {
		...
		if(isset($template_id)) {
			...
			if( $type == 'image' ){
				echo '<div id="'. ebor_sanitize_title($title) .'" class="parallax" style="background-image: url('. $image .');"><div class="container inner">';
			} else {
				echo '<div id="'. ebor_sanitize_title($title) .'" class="' . $type . '"><div class="container inner">';
			}
			
			echo ( 1 == $thin ) ? '<div class="thin">' : '<div class="row">';
