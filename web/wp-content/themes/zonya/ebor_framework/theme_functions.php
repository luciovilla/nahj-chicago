...
if(!( function_exists('ebor_portfolio_filters') )){
	function ebor_portfolio_filters($cats = false){
	?>
	
		<ul class="filter">
			<li><a class="active" href="#" data-filter="*"><?php _e('All','zonya'); ?></a></li>
			<?php 
				foreach ($cats as $cat){
...
if(!( function_exists('tcb_add_post_thumbnail_column') )){
	function tcb_add_post_thumbnail_column($cols){
	  $cols['tcb_post_thumb'] = __('Featured Image','zonya');
	  return $cols;
	}
}
...
if(!( function_exists('ebor_pagination') )){
	function ebor_pagination($pages = '', $range = 2){
		...
		if(1 != $pages){
			...
			if($paged > 2 && $paged > $range+1 && $showitems < $pages) $output .= "<li><a href='".get_pagenum_link(1)."'><i class='icon-left-open-big'></i></a></li> ";
			...
			if ($paged < $pages-1 &&  $paged+$range-1 < $pages && $showitems < $pages) $output .= "<li><a href='".get_pagenum_link($pages)."'><i class='icon-right-open-big'></i></a></li> ";
			$output.= "</ul></div>";
		}
		...
		<li <?php comment_class(); ?> id="comment-<?php comment_ID() ?>">
		  ...
		  <div class="message">
		    <div class="box">
		      ...
		      <p><em><?php _e('Your comment is awaiting moderation.', 'zonya') ?></em></p>
		      <?php endif; ?>
		    </div>
		  </div>
		</li>
